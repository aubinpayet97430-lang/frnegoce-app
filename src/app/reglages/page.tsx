...
import { Save, User, Building2, Mail, Hash, CheckCircle } from 'lucide-react'
...
export default function ReglagesPage() {
  ...
  async function sauvegarder() {
    ...
    const supabase = createClient()
    if (profil.id) {
      await supabase.from('profil_utilisateur').update({
        nom: profil.nom,
      ...
      const { data } = await supabase.from('profil_utilisateur').insert({
        nom: profil.nom,
        adresse: profil.adresse,
        siret: profil.siret,
        email: profil.email,
    ...
    <>
      ...
      <main className="max-w-2xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Vos informations utilisées sur les factures PDF</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex : 1 rue de la Coopérative, 97400 Saint-Denis"
              className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
          </div>
          ...
          <button
            onClick={sauvegarder}
            disabled={saving || !profil.nom.trim()}
        ...
        <p className="text-xs text-gray-400 text-center mt-4">
          Ces informations apparaissent automatiquement sur toutes vos factures de commission PDF.
        </p>
      </main>
    </>
  )
